...
namespace rapidoson {
    ...
    struct ToJsonValue<ConfigType, typename std::enable_if<std::is_arithmetic<ConfigType>::value>::type> {
        static std::string Convert(const ConfigType& c) {
            return fmt::format(R"({})", c);
        }
    };
    ...
    void TestLeafConstraints(T t, bool expect_success = true, const std::string &expected_message = "") {
